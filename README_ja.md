...
- [scripts/WAN0_ipv6privacy.sh](scripts/WAN0_ipv6privacy.sh) : WAN 側の autoconfprivacy な IPv6 を取得してファイルに書き出すスクリプト
...
WAN0 は `inet6 autoconf autoconfprivacy` で設定します。
...
autoconfprivacy で設定された IPv6 は一定時間で無効になるので、新たな IPv6 address が assign されたらそのアドレスを使うように [scripts/gwi_address.sh](scripts/gwi_address.sh) を cron で適当な間隔で実行されるように設定します。
